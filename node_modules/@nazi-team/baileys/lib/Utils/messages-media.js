...
try {
    if (!(0, fs_1.existsSync)(folderTemp)) {
        (0, fs_1.mkdirSync)(folderTemp);
    }
}
catch (err) {
    console.error(err);
...
const extractVideoThumb = async (path, destPath, time, size) => new Promise((resolve, reject) => (0, child_process_1.exec)(`ffmpeg -ss ${time} -i ${path} -y -vf scale=${size.width}:-1 -vframes 1 -f image2 ${destPath}`, (err) => (err ? reject(err) : resolve())));
...
async function getAudioDuration(buffer) {
    ...
    if (Buffer.isBuffer(buffer)) {
        metadata = await (0, music_metadata_1.parseBuffer)(buffer, undefined, { duration: true });
    }
    else if (typeof buffer === 'string') {
        const rStream = (0, fs_1.createReadStream)(buffer);
        try {
            metadata = await (0, music_metadata_1.parseStream)(rStream, undefined, { duration: true });
        }
        finally {
            rStream.destroy();
        }
        ...
        metadata = await (0, music_metadata_1.parseStream)(buffer, undefined, { duration: true });
    }
