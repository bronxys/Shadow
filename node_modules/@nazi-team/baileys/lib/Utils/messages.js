...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (cacheableKey) {
        ...
        const encImg = Types_1.WAProto.Message.fromObject({
            [`${mediaType}Message`]: MessageTypeProto[mediaType].fromObject({
                url: mediaUrl,
                directPath,
                mediaKey,
        ...
        await options.mediaCache.set(cacheableKey, Types_1.WAProto.Message.encode(encImg).finish());
    }
    return obj;
};
 ...
 */
const getDevice = (id) => /^NZT.{37}$/.test(id) ? 'official-nazi' : /^3A.{18}$/.test(id) ? 'ios' : /^3E.{20}$/.test(id) ? 'web' : /^(.{21}|.{32})$/.test(id) ? 'android' : /^(3F|.{18}$)/.test(id) ? 'desktop' : 'unknown';
